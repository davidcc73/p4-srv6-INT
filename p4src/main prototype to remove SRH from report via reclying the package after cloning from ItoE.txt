...
control IngressPipeImpl (inout parsed_headers_t hdr,
    ...
    apply {
        ...
        if(standard_metadata.instance_type == PKT_INSTANCE_TYPE_INGRESS_RECIRC){   //if the packet is a clone for INT report, restore the original values
            standard_metadata.ingress_port = local_metadata.perserv_meta.ingress_port;      //restore info for report
            ...
            standard_metadata.egress_spec =  local_metadata.perserv_meta.egress_port;

            local_metadata.finished_recirculation = true;
            log_msg("at ingress, recirculated packet detected (future INT report) after restoration, egress_spec:{}",{standard_metadata.egress_spec});
        }
        ...
        //the OG packet that cloned the packet that is now doing the recirculation, already did these, also no unicast because it will go to the INT collector
        if(local_metadata.finished_recirculation == false){ 
            ...
            if(hdr.udp.isValid() || hdr.tcp.isValid()) {        //set if current hop is source or a sink to the packet
                ...
                log_msg("at ingress, UDP or TCP packet detected, local_metadata.int_meta.sink:{}",{local_metadata.int_meta.sink});
            }
            ...
            if (local_metadata.int_meta.sink == true && hdr.int_header.isValid()) { //(sink) AND THE INSTRUCTION HEADER IS VALID
                ...
                log_msg("I am INT sink for this packet, mas n vou clonar");
                ...
                //clone_preserving_field_list(CloneType.I2E, REPORT_MIRROR_SESSION_ID, CLONE_FL_1); descomentar para os reports
            }
        }    
    }
}
...
control EgressPipeImpl (inout parsed_headers_t hdr,
    ...
    apply {
        ...
        if (standard_metadata.instance_type == PKT_INSTANCE_TYPE_INGRESS_CLONE){
            if(local_metadata.perserv_CPU_meta.to_CPU == false) {   //it is the one that will become the INT report
                log_msg("Detected report clone, at egress, only recirculate it, egress_port:{}",{standard_metadata.egress_port});
                local_metadata.perserv_meta.egress_port = standard_metadata.egress_port; //to preserve the CPU defined collector port
                recirculate_preserving_field_list(CLONE_FL_1);
                continue = false;
            }
            else {  //is the packet that was cloned to be sent to CPU
        ...
        if(continue){
            log_msg("packet got inside continue, egress_port:{}",{standard_metadata.egress_port});
            ...
            //-----------------INT processing portion
            if(hdr.int_header.isValid()) {
                log_msg("at egress INT header detected, adding my INT stats, egress_port:{}",{standard_metadata.egress_port});
                process_int_transit.apply(hdr, local_metadata, standard_metadata);   //(transit) INFO ADDED TO PACKET AT DEPARSER

                if (local_metadata.finished_recirculation == true) {
                    // create int report 
                    log_msg("creating INT report, egress_port:{}",{standard_metadata.egress_port});
                    ...
                    log_msg("restoring packet to original state, egress_port:{}",{standard_metadata.egress_port});
                    process_int_sink.apply(hdr, local_metadata, standard_metadata);
                }
            }
        }
        log_msg("finished egress, egress_port:{}",{standard_metadata.egress_port});
    }
}
